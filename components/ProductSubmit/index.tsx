...
const ProductSubmit = () => {
	...
	const handleChange = name => event => {
		if (name === 'is_eco_friendly')
			setFormValue({ ...formValue, [name]: Boolean(event.target.value) });
		else if (name === 'price' || name === 'carbon_emission')
			setFormValue({ ...formValue, [name]: Number(event.target.value) });
	...
	const productValidationCheck = async () => {
		...
		await fetch(`${baseUrl}/products`, {
			...
			body: JSON.stringify({
				...formValue,
				image_link: [
					'https://previews.123rf.com/images/baldyrgan/baldyrgan1309/baldyrgan130900283/22348504-%EA%B7%B8%EB%A6%B0-%EC%97%90%EC%BD%94-%EC%97%90%EB%84%88%EC%A7%80-%EA%B0%9C%EB%85%90-%EC%8B%9D%EB%AC%BC%EC%9D%80-%EC%A0%84%EA%B5%AC-%EC%95%88%EC%97%90-%EC%84%B1%EC%9E%A5.jpg',
				],
	...
	return (
		<ProductSubmitForm noValidate autoComplete="off">
			...
			<TextFieldInput
				...
				label="name"
				error={formValue.name.length !== 0 && !validationCheck('name')}
				onChange={handleChange('name')}
				helperText="제품 이름은 2자~20자 이내로 입력해주세요."
			/>
			<TextFieldInput
				id="standard-basic-brand"
				label="brand"
			...
			<FormControlContainer>
				...
				>
					{/* {CategorySection} */}
					<MenuItem key={`1`} value={`2`}>{`식품> 생수/음료`}</MenuItem>;
				</Select>
			</FormControlContainer>
			<TextFieldInput
			...
			/>
			<TextFieldInput
				id="standard-basic-carbon_emission"
				label="carbon_emissions (option)"
				type="number"
				onChange={handleChange('carbon_emission')}
			/>
			<FormLabel style={{ textAlign: 'left', marginTop: '20px' }}>
				is_eco
			</FormLabel>
			<RadioGroup
				aria-label="eco"
				id="standard-basic-is-eco-friendly"
				name="is_eco_friendly"
				defaultValue="true"
				onChange={handleChange('is_eco_friendly')}
			>
			...
			<Button
				variant="contained"
				type="submit"
				onClick={productValidationCheck}
				disabled={
					!validationCheck('name') ||
					!validationCheck('brand') ||
					!validationCheck('category_id') ||
